...
df2$Dalc <- mapvalues(df2$Dalc, 
                              from = 1:5, 
                              to = c("Very Low", "Low", "Medium", "High", "Very High"))
base1<-ggplot(df1,aes(x=Dalc))
```


```{r}
base1+geom_bar(aes(y= pass_fail=TRUE))
```
...
```{r}
df1lm = lm(grade_mean ~.-G1 -G2 -G3 -pass_fail,data=train)
summary(df1lm)
```
...
```{r}
df1LOG = glm(train$pass_fail~.- school - sex -age, data = train, family = "binomial")
```
